...
#     Actually |z_1| = sqrt(3.25) ≈ 1.80 < 2, so does NOT escape at k=0.
...
# ---------------------------------------------------------------------------
# The naive double loop is the simplest correct implementation — it is the
...
# in the escape condition (it uses z.real*z.real + z.imag*z.imag > 4.0
# rather than abs(z) > 2, which is mathematically identical but worth
# verifying produces the same pixel-level results).
#
# A warm-up call is included so Numba JIT compilation happens before the
# tested call — compilation is not part of correctness.
...
def test_parallel_matches_naive_oracle() -> None:
    ...
    mandelbrot_parallel(4, 4, max_iter)          # warm-up: trigger JIT compile
